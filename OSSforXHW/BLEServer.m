...
-(void)addPeripheralInfo:(PeriperalInfo *)peripheralInfo
{
    ...
    if ([peripheralInfo.name isEqualToString:@"AUTODA"]) {
...
//        for (CBService *s in peripheral.services) {
                ...
                [self discoverService:[peripheral.services lastObject]];
                NSLog(@"连接服务成功了");
//                           }
...
{
     NSLog(@"-----tezheng----%@",service.characteristics);
    
    
    if (nil == error) {
        characteristicState = KSUCCESS;
        self.discoveredSevice = service;
//        [self notifyValue:[service.characteristics objectAtIndex:2]];
//        [self readValue:[service.characteristics objectAtIndex:2]];
        NSLog(@"-----tezheng-监听特征---%@",[service.characteristics objectAtIndex:2]);
//        for (CBCharacteristic *c  in service.characteristics) {
//            if ([c.UUID isEqual:@"49535343-1E4D-4BD9-BA61-23C647249616"]) {
//                NSLog(@"-----tezheng----%@",c);
//                
                [self notifyValue:[service.characteristics objectAtIndex:2]];
                [self readValue:[service.characteristics objectAtIndex:2]];
