...
- (void)sendData:(id)data responseCallback:(WVJBResponseCallback)responseCallback handlerName:(NSString*)handlerName {
    ...
    if (responseCallback) {
        NSString* callbackId = [NSString stringWithFormat:@"JAVA_CB_%ld", ++_uniqueId];
...
- (void)flushMessageQueue:(NSString *)messageQueueString{
    ...
    for (WVJBMessage* message in messages) {
        ...
        if (responseId) {
            ...
            {
                //dengzhcuai
                if ([message[@"data"] isEqual:[NSNull null]])
                {
                    handler = self.messageHandler;
                }
                else
...
- (void)injectJavascriptFile:(BOOL)shouldInject {
    if(shouldInject){
        ...
        NSString *filePath = [bundle pathForResource:@"MyNative" ofType:@"js"];
        NSString *js = [NSString stringWithContentsOfFile:filePath encoding:NSUTF8StringEncoding error:nil];
        [self _evaluateJavascript:js];
        [self dispatchStartUpMessageQueue];
        NSLog(@"MyNative.js---ok");
    }
}
...
-(NSString *)webViewJavascriptCheckCommand {
    return @"typeof MyNative == \'object\';";
}

-(NSString *)webViewJavascriptFetchQueyCommand {
    return @"MyNative._fetchQueue();";
}
...
//    NSString* javascriptCommand = [NSString stringWithFormat:@"WebViewJavascriptBridge._handleMessageFromNative('%@');", messageJSON];
        NSString* javascriptCommand = [NSString stringWithFormat:@"MyNative._handleMessageFromObjC('%@');", messageJSON];
