...
//  News
//
//  Created by dengzhucai on 14-12-7.
//  Copyright (c) 2014年 dengzhucai. All rights reserved.
//
...
+ (void)animationWithFadeIn:(UIView *)view endAlpha:(float)alpha duration:(CFTimeInterval)duration
{
    view.alpha = 0;
    [UIView animateWithDuration:duration animations:^{
        view.alpha = alpha;
...
+ (void)animationWithFadeOut:(UIView *)view duration:(CFTimeInterval)duration endRemove:(NSArray *)views
{
    [UIView animateWithDuration:duration animations:^{
        view.alpha = 0;
 ...
 *
 *  @param view             动画持续时间,值得一提的是,如果添加到group里的子动画不设置此属性,group里的duration会统一
